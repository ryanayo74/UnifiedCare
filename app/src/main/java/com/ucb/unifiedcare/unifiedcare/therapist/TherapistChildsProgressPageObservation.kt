...
import Adapter.PlanNextSessionAdapter
import android.os.Bundle
import android.widget.TextView
import androidx.appcompat.app.AppCompatActivity
import com.ucb.unifiedcare.R
import com.ucb.unifiedcare.databinding.ActivityTherapistChildsProgressPageObservationBinding
import java.text.SimpleDateFormat
import java.util.Calendar
import java.util.Locale

class TherapistChildsProgressPageObservation : AppCompatActivity() {
    private lateinit var binding: ActivityTherapistChildsProgressPageObservationBinding
    private var currentWeekOffset = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityTherapistChildsProgressPageObservationBinding.inflate(layoutInflater)
        ...
        val observationTitles = listOf("Add Session")
        // Set up CustomAdapter with placeholder data
        val observationAdapter = ObservationAdapter(this, observationTitles)
        binding.listview.adapter = observationAdapter

        // Placeholder data to simulate a database content
        val nextSessionTitles = listOf("Add Session")
        // Set up CustomAdapter with placeholder data
        val nextSessionAdapter = PlanNextSessionAdapter(this, nextSessionTitles)
        binding.listview.adapter = nextSessionAdapter
